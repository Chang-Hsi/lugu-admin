<template>
  <div class="p-2 md:p-4 space-y-2">
    ...
    <div class="flex items-center justify-between">
      <div class="text-lg font-bold text-slate-700">問卷調查</div>
    ...
    <el-card shadow="never" class="border! border-gray-200!">
      ...
      >
        ...
        <div class="flex gap-2 ml-auto">
          ...
          <!-- 新增問卷 -->
          <el-button type="success" @click="openEditor()">新增問卷</el-button>
        </div>
      </el-form>
    </el-card>
    ...
    <el-card shadow="never" class="border! border-gray-200!">
      ...
      >
        ...
        <el-table-column label="問卷名稱" min-width="420" show-overflow-tooltip>
        ...
        <el-table-column label="項目格式" width="160" align="center" fixed="right">
          <template #default="{ row }">
            <el-button @click="openEditor(row)">編輯問卷</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog
      ...
      :title="editor.isNew ? '新增問卷' : '編輯問卷'"
      width="900px"
      top="5vh"
    >
      <el-form :model="editor.form" label-width="96px" class="space-y-2">
        <el-form-item label="問卷名稱">
          <el-input v-model="editor.form.title" placeholder="請輸入問卷名稱" />
        </el-form-item>
...
function seedIfEmpty() {
  ...
  for (let i = 0; i < 16; i++) {
    ...
    list.push({
      id: nid(),
      title,
      summary: i % 2 === 0 ? "此問卷旨在蒐集中肯建議，協助優化公共服務。" : "",
...
async function saveSurvey() {
  const f = editor.form;
  if (!f.title?.trim()) return ElMessage.error("請輸入問卷名稱");
  if (editor.unlimited) {
    f.endAt = null;
